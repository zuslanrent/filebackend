...
const getAuditLogs = async (req, res) => {
  try {
    const { search, department, action } = req.query
...
const createAuditLog = async (req, res) => {
  try {
    const { file_id, file_name, user_id, user_name, user_department, action, details } = req.body

    const result = await pool.query(
      `INSERT INTO audit_logs (file_id, file_name, user_id, user_name, user_department, action, details)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [file_id, file_name, user_id, user_name, user_department, action, details || null]
    )
